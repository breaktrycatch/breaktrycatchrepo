	...
	public class NumberDisplay extends FuelUI 
	{
			...
			if ( !isNaN( num ) )
			{
				...
				for ( var i : Number = len; i > 0 ; i-- )
				{
					if( __useComma && i % 3 == 0 && ( len - i ) != 0 )
					{
						...
						comma.x = ( mc.width + __spacing ) * ( len - i ) + commas.length * mc.width;
						
						commas.push( commas );
						__numbers.push( comma );
					}
					
					var mc : MovieClip = createChar();
					
					addChild( mc );
					
					mc.x = ( mc.width + __spacing ) * ( len - i ) + commas.length * mc.width;
		...
		protected function clearNumbers() : void
		{
			if( __numbers.length )
			{
				do
				{
					var mc : MovieClip = __numbers.shift( );
