	...
	public class BaseList extends FuelUI
	{
		...
		protected function createRows() : void
		{
			...
			for( i = 0;i < len; i++ )
			{
				...
				{
					var exp : RegExp = /row\d+/;
					var test : Boolean = exp.test(instancename);
					if( test )
					{
						//Original
						var pattern : RegExp = /\d+/;
						var index : int = int(instancename.match(pattern));
                                          
						//Create new AssetProxy of Type
						var cell : AssetProxy = new __cellRendererType();
						
						//Set display by Type
						cell.setDisplayByClip(MovieClip(child));
						__rows[ index ] = cell;
						initializeCellRenderer(cell as ICellRenderer);
					}
				}
			}
			...
			switch( type )
			{
				...
				case DataProviderEvent.REPLACE_ITEM:
					if( index >= __startIndex && index < ( __startIndex + __rowCount ) )
					{
						...
						row.cellData = __dataProvider.getItemAt(index) as ICellRendererData;
					} 
					break;
		...
		protected function invalidateList( startIndex : int = 0 ) : void
		{
			for( var i : int = startIndex; i < __rowCount; i++ )
			{
				...
				{
					var item:ICellRendererData = __dataProvider.getItemAt(index) as ICellRendererData;
					setRow(item, index, i);
				}
			}			
		}
		 ...
		 * 
		 */
		protected function setRow( item : ICellRendererData, index : int, rowindex : int ) : void
		{
			var row : ICellRenderer = __rows[ rowindex ] as ICellRenderer;
			
			if( item is ICellRendererData )
			{
				if( item != null )
				{
			...
			{
				throw new TypeError( "Object at index " + index + " in list dataProvider is not a ICellRendererData Object" );	
			}
		}
		 ...
		 * 
		 */
		public function addItem( item : ICellRendererData ) : void
		{
		 ...
		 * 
		 */
		public function addItemAt( item : ICellRendererData, index : int ) : void
		{
		 ...
		 * 
		 */
		public function replaceItemAt( item : ICellRendererData, index : int ) : void
		{
		 ...
		 * 
		 */
		public function getItemAt( index : int ) : ICellRendererData
		{
			return( __dataProvider.getItemAt(index) as ICellRendererData );	
		}
	}
}