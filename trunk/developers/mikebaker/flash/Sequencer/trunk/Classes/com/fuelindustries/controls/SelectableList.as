package com.fuelindustries.controls
{
	 ...
	 * 
	 * @eventType com.fuelindustries.events.ListEvent.ITEM_CLICK
	 * 
	 */
	[Event(name="itemClick", type="com.fuelindustries.events.ListEvent")]
	
	/**
	 * Dispatched when the user rolls over an item in the component. 
	 * 
	 * @eventType com.fuelindustries.events.ListEvent.ITEM_ROLL_OVER
	 * 
	 */
	[Event(name="itemRollOver", type="com.fuelindustries.events.ListEvent")]
	
	/**
	 * Dispatched when the user rolls out of an item in the component.
	 * 
	 * @eventType com.fuelindustries.events.ListEvent.ITEM_ROLL_OUT
	 * 
	 */
	[Event(name="itemRollOut", type="com.fuelindustries.events.ListEvent")]
	...
	{		
		...
		/** @private */
		protected var __selectedItem:ICellRendererData;
		/** @private */
		...
		protected function rowClick( event:MouseEvent ):void
		{
			...
			__selectedItem = row.cellData as ICellRendererData;
			__selectedRow = row;
			
			dispatchEvent( new ListEvent( ListEvent.ITEM_CLICK, __selectedItem, index, rowIndex ) );
		}
		...
		protected function rowOver( event:MouseEvent ):void
		{
			...
			var rowIndex:int = listData.row;
			var index:int = listData.index;
			dispatchEvent( new ListEvent( ListEvent.ITEM_ROLL_OVER, row.cellData, index, rowIndex ) );
		}
		...
		protected function rowOut( event:MouseEvent ):void
		{
			...
			var rowIndex:int = listData.row;
			var index:int = listData.index;
			dispatchEvent( new ListEvent( ListEvent.ITEM_ROLL_OUT, row.cellData, index, rowIndex ) );
		}
		
		/** @inheritDoc */
		override protected function setRow( item:ICellRendererData, index:int, rowindex:int ):void
		{
